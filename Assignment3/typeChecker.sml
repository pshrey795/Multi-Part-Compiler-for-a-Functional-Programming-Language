...
fun envLookup(i: id, env:typeEnv) = 
case List.find(fn (x,_) => x = i) env of
 SOME(ex,t) => t
 | NONE     => raise nonDecl

fun envAdd(i:id, t: typ, env:typeEnv) = (i,t)::env
...
| BinExp(b,e1,e2)		=> let 
				   ...
				   in
				   	...
				   	| RelOp(x)    => if t1=BOOL then (INT,env) else raise invalidType
				   end 
| LetExp(ValDecl(i,e1),e2)      => typeCheck(e2,envAdd(i,let val (t,_) = typeCheck(e1,env) in t end,env))
| AppExp(e1,e2)                 => let 
				   	val (t1,_) = typeCheck(e1,env)
				   	val (t2,_) = typeCheck(e2,env)
				   in 
...
| DeclExp(fd,e1)		=> case fd of
				   Fn(i,t1,t2)	      => let
				   		   		val (t,_) = typeCheck(e1,envAdd(i,t1,env))
				   		   	 in
				   		   		if t=t2 then (t,env) else raise invalidType
				   		   	 end
				   | Fun(i1,i2,t1,t2) => let
				   		   		val (t,_) = typeCheck(e1,envAdd(i2,t1,env))
				   		   	 in
				   		   		if t=t2 then (t,envAdd(i1,t,env)) else raise invalidType
				   		   	 end
